...
const API_URL = "http://127.0.0.1:5000/api/yt-stats"

const App = () => {
  const [statsData, setStatsData] = useState([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState(null)
  const [activeTab, setActiveTab] = useState("efficiency") // 'efficiency' | 'subscribers' | 'content'
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!response.ok)
          throw new Error("Could not connect to the data stream.")
...
// Sub-components for a cleaner App file
...
const LoadingScreen = () => (
  <div className="min-h-screen bg-[#020617] flex flex-col items-center justify-center">
    ...
    <p className="text-slate-500 font-mono text-xs tracking-[0.3em] animate-pulse uppercase">
      Syncing with API...
    </p>
  </div>
)
...
const ErrorScreen = ({ error }) => (
  <div className="min-h-screen bg-[#020617] flex flex-col items-center justify-center p-6 text-center">
    ...
    <p className="text-slate-500 max-w-sm text-sm mb-8">{error}</p>
    <button
      onClick={() => window.location.reload()}
      className="px-8 py-3 bg-white text-black font-black uppercase text-xs tracking-widest rounded-full hover:bg-indigo-500 hover:text-white transition-all"
    >
      Retry Connection
    </button>
  </div>
